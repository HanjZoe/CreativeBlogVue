                                ...
                                <li class="post">
                                    <a href="{{route('post.show',$likedPost->id)}}" class="post-permalink media">
                                        ...
                                        <div class="media-body">
                                            <h6 class="post-title">{{$likedPost->category->title}}</h6>
                                            @auth()
                                                <form action="{{route('post.like.store',$post->id)}}" method="post">
                                                    @csrf
                                                    <span>
                                        {{$likedPost->liked_users_count}}
                                    </span>
                                                    <button class=" border-0 bg-transparent" type="submit">
                                                        <i class="fa{{auth()->user()->likedPosts->contains($likedPost->id) ? "s" : "r"}} fa-heart"></i>
                                                    </button>
                                                </form>
                                            @endauth
                                            @guest()
                        ...
                        @if(@count($categories) > 0)
                            <ul>
                                @foreach($categories as $category)
                                    <li><a class="widget-title" href="{{route('category.post.index',$category->id)}}">{{$category->title}}</a></li>
                                @endforeach
                            </ul>
                        @else
                            <a>Нет категорий</a>
